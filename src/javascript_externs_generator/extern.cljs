...
(defn wrap-extern-remix
  [namespace-text file-list extern]
  (.warn js/console file-list)
  (let [files (string/join \newline (mapv #(str " *   " %) file-list))
        header (str "/**********************************************************************\n"
                    (gstring/format " * Extern for %s\n" namespace-text)
                    (gstring/format " * Dependencies:\n%s\n" files)
                    " * Generated by http://tomjkidd.github.io/javascript-externs-generator\n"
                    "/**********************************************************************/\n")
        footer (str "/**********************************************************************\n"
                    (gstring/format " * End Generated Extern for %s\n" namespace-text)
                    "/**********************************************************************/\n")]
    (str header extern "\n" footer)))

(defn extract-loaded-remix
  [namespace-text file-list]
  (let [sandbox (dom/getElement "sandbox")
        js-object (-> sandbox (obj/get "contentWindow") (obj/get namespace-text))]
    (when (nil? js-object)
      (throw (gstring/format "Namespace '%s' was not found. Make sure that the loaded libraries are in correct sequential order, and that the name is spelled correctly."
                             namespace-text)))
    (wrap-extern-remix namespace-text file-list (extract namespace-text js-object))))
